...
def convertExampleFRMs():
	"""Convert all FRMs in example_frm/ to PNG, output to example_frm/out/"""
	BASE_DIR    = r"C:\Users\aaron\OneDrive\Dokumente\GitHub\DH3D-Sprites\SpriteDarkHarold3D"
	palettePath = os.path.join(BASE_DIR, 'color', 'color.pal')
	frmDir      = os.path.join(BASE_DIR, 'example_frm')
	outDir      = os.path.join(BASE_DIR, 'example_frm', 'out')
	...
	for frm_path in glob.glob(os.path.join(frmDir, "*.frm")):
		name = os.path.splitext(os.path.basename(frm_path))[0].lower()
		out_path = os.path.join(outDir, name + ".png")
		frmpixels.exportFRM(frm_path, out_path, palette)
		print(f"  {name}.png")
